#!/bin/bash
# 356-show-static.sh -- End-to-end: netfyr show with static-only policies.
#
...
if [[ ! -x "$NETFYR_BIN" ]]; then
    echo "FAIL: 356-show-static: netfyr binary not found at $NETFYR_BIN" >&2
    exit 1
fi
if [[ ! -x "$NETFYR_DAEMON_BIN" ]]; then
    echo "FAIL: 356-show-static: netfyr-daemon binary not found at $NETFYR_DAEMON_BIN" >&2
    exit 1
fi
...
while [[ ! -S "$SOCKET_PATH" ]]; do
    if ! kill -0 "$DAEMON_PID" 2>/dev/null; then
        echo "FAIL: 356-show-static: daemon exited before socket appeared" >&2
        exit 1
    fi
    if (( SOCKET_WAIT >= 50 )); then
        echo "FAIL: 356-show-static: daemon socket did not appear within 5 seconds" >&2
        exit 1
    fi
    sleep 0.1
    (( SOCKET_WAIT++ )) || true
...
if [[ $APPLY_EXIT -ne 0 ]]; then
    echo "FAIL: 356-show-static: netfyr apply exited with code $APPLY_EXIT" >&2
    exit 1
fi
...
if ! echo "$SHOW_OUTPUT" | grep -q "Status:  running"; then
    echo "FAIL: 356-show-static: show output does not contain 'Status:  running'" >&2
...
if ! echo "$SHOW_OUTPUT" | grep -q "Uptime:"; then
    echo "FAIL: 356-show-static: show output does not contain 'Uptime:' line" >&2
    echo "      output: $SHOW_OUTPUT" >&2
    exit 1
fi
...
if ! echo "$SHOW_OUTPUT" | grep -q "veth-e2e0"; then
    echo "FAIL: 356-show-static: show output does not contain 'veth-e2e0'" >&2
    echo "      output: $SHOW_OUTPUT" >&2
    exit 1
fi
if ! echo "$SHOW_OUTPUT" | grep -q "veth-e2e2"; then
    echo "FAIL: 356-show-static: show output does not contain 'veth-e2e2'" >&2
    echo "      output: $SHOW_OUTPUT" >&2
    exit 1
fi
...
if [[ $STATIC_COUNT -lt 2 ]]; then
    echo "FAIL: 356-show-static: expected at least 2 '(static)' policy entries, got $STATIC_COUNT" >&2
    echo "      output: $SHOW_OUTPUT" >&2
    exit 1
fi
...
if echo "$SHOW_OUTPUT" | grep -q "DHCP:"; then
    echo "FAIL: 356-show-static: show output unexpectedly contains 'DHCP:' line" >&2
    echo "      output: $SHOW_OUTPUT" >&2
    exit 1
fi
if echo "$SHOW_OUTPUT" | grep -q "Lease:"; then
    echo "FAIL: 356-show-static: show output unexpectedly contains 'Lease:' line" >&2
    echo "      output: $SHOW_OUTPUT" >&2
    exit 1
fi

echo "PASS: 356-show-static"
