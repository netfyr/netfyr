#!/bin/bash
# 600-e2e-dhcp-journal-per-interface.sh
# End-to-end: DHCP events on multiple interfaces produce separate journal
# entries, each scoped to a single interface.
#
# Topology:
...
#   bash tests/600-e2e-dhcp-journal-per-interface.sh
set -euo pipefail
...
if [[ ! -x "$NETFYR_BIN" ]]; then
    echo "FAIL: 600-e2e-dhcp-journal-per-interface: netfyr binary not found at $NETFYR_BIN" >&2
    exit 1
fi
if [[ ! -x "$NETFYR_DAEMON_BIN" ]]; then
    echo "FAIL: 600-e2e-dhcp-journal-per-interface: netfyr-daemon binary not found at $NETFYR_DAEMON_BIN" >&2
    exit 1
fi
if ! command -v dnsmasq >/dev/null 2>&1; then
    echo "FAIL: 600-e2e-dhcp-journal-per-interface: dnsmasq not found; install dnsmasq to run DHCP tests" >&2
    exit 1
fi
if ! command -v jq >/dev/null 2>&1; then
    echo "FAIL: 600-e2e-dhcp-journal-per-interface: jq not found" >&2
    exit 1
fi
...
while [[ ! -S "$SOCKET_PATH" ]]; do
    if ! kill -0 "$DAEMON_PID" 2>/dev/null; then
        echo "FAIL: 600-e2e-dhcp-journal-per-interface: daemon exited before socket appeared" >&2
        exit 1
    fi
    if (( SOCKET_WAIT >= 50 )); then
        echo "FAIL: 600-e2e-dhcp-journal-per-interface: daemon socket did not appear within 5 seconds" >&2
        exit 1
    fi
    sleep 0.1
    (( SOCKET_WAIT++ )) || true
...
if [[ $APPLY_EXIT -ne 0 ]]; then
    echo "FAIL: 600-e2e-dhcp-journal-per-interface: netfyr apply exited with code $APPLY_EXIT" >&2
    exit 1
fi
...
if [[ ! -f "$JOURNAL_FILE" ]]; then
    echo "FAIL: 600-e2e-dhcp-journal-per-interface: journal file not found at $JOURNAL_FILE" >&2
    exit 1
fi
...
if [[ "$ACQUIRED_COUNT" -lt 2 ]]; then
    echo "FAIL: 600-e2e-dhcp-journal-per-interface: expected at least 2 lease_acquired entries, got $ACQUIRED_COUNT" >&2
    echo "      journal contents:" >&2
    cat "$JOURNAL_FILE" >&2
    exit 1
fi

# For each lease_acquired entry, verify it contains exactly one entity in the diff
# and exactly one entity in state_after.
ENTRY_IDX=0
while IFS= read -r entry; do
    ENTRY_IDX=$((ENTRY_IDX + 1))

    # Count non-system entity names in diff operations.
    ...
    if [[ "$DIFF_ENTITY_COUNT" -gt 1 ]]; then
        ...
        echo "FAIL: 600-e2e-dhcp-journal-per-interface: lease_acquired entry #$ENTRY_IDX has $DIFF_ENTITY_COUNT entities in diff ($DIFF_ENTITIES), expected at most 1" >&2
        echo "      entry:" >&2
        echo "$entry" | jq . >&2
        exit 1
    fi

    # Count entities in state_after.
    ...
    if [[ "$STATE_ENTITY_COUNT" -gt 1 ]]; then
        ...
        echo "FAIL: 600-e2e-dhcp-journal-per-interface: lease_acquired entry #$ENTRY_IDX has $STATE_ENTITY_COUNT entities in state_after ($STATE_ENTITIES), expected at most 1" >&2
        echo "      entry:" >&2
        echo "$entry" | jq . >&2
        exit 1
    fi
...
# Verify both interfaces appear across the entries (one per entry).
...
if [[ "$IFACE0_FOUND" -lt 1 ]]; then
    echo "FAIL: 600-e2e-dhcp-journal-per-interface: no lease_acquired entry for dhcp-iface0" >&2
    exit 1
fi
if [[ "$IFACE2_FOUND" -lt 1 ]]; then
    echo "FAIL: 600-e2e-dhcp-journal-per-interface: no lease_acquired entry for dhcp-iface2" >&2
    exit 1
fi

echo "PASS: 600-e2e-dhcp-journal-per-interface"
