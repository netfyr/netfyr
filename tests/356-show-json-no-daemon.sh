#!/bin/bash
# 356-show-json-no-daemon.sh -- End-to-end: netfyr show -o json produces valid JSON when daemon is not running.
#
...
if [[ ! -x "$NETFYR_BIN" ]]; then
    echo "FAIL: 356-show-json-no-daemon: netfyr binary not found at $NETFYR_BIN" >&2
    exit 1
fi
if ! command -v jq >/dev/null 2>&1; then
    echo "FAIL: 356-show-json-no-daemon: jq not found; install jq to run JSON validation tests" >&2
    exit 1
fi
...
if [[ $SHOW_EXIT -ne 0 ]]; then
    echo "FAIL: 356-show-json-no-daemon: netfyr show -o json exited with code $SHOW_EXIT (expected 0)" >&2
...
if ! echo "$JSON_OUTPUT" | jq -e 'type == "object"' >/dev/null 2>&1; then
    echo "FAIL: 356-show-json-no-daemon: output is not a valid JSON object" >&2
    echo "      output: $JSON_OUTPUT" >&2
    exit 1
fi
...
if [[ "$DAEMON_STATUS" != "not_running" ]]; then
    echo "FAIL: 356-show-json-no-daemon: daemon.status is '$DAEMON_STATUS', expected 'not_running'" >&2
    exit 1
fi
...
if [[ "$HAS_UPTIME" != "false" ]]; then
    echo "FAIL: 356-show-json-no-daemon: daemon object must not have 'uptime_seconds' when not running" >&2
...
if [[ "$IFACE_COUNT" -lt 1 ]]; then
    echo "FAIL: 356-show-json-no-daemon: interfaces array is empty" >&2
    exit 1
fi
...
if [[ "$HAS_POLICIES" != "false" ]]; then
    echo "FAIL: 356-show-json-no-daemon: some interface has 'policies' field (expected bare name only)" >&2
...
if [[ "$HAS_DHCP" != "false" ]]; then
    echo "FAIL: 356-show-json-no-daemon: some interface has 'dhcp' field (expected bare name only)" >&2
    echo "      interfaces: $(echo "$JSON_OUTPUT" | jq '.interfaces')" >&2
    exit 1
fi

echo "PASS: 356-show-json-no-daemon"
