#!/bin/bash
# 356-show-multi-policy.sh -- End-to-end: netfyr show with multiple policies on one interface.
#
...
if [[ ! -x "$NETFYR_BIN" ]]; then
    echo "FAIL: 356-show-multi-policy: netfyr binary not found at $NETFYR_BIN" >&2
    exit 1
fi
if [[ ! -x "$NETFYR_DAEMON_BIN" ]]; then
    echo "FAIL: 356-show-multi-policy: netfyr-daemon binary not found at $NETFYR_DAEMON_BIN" >&2
    exit 1
fi
if ! command -v dnsmasq >/dev/null 2>&1; then
    echo "FAIL: 356-show-multi-policy: dnsmasq not found; install dnsmasq to run DHCP tests" >&2
    exit 1
fi
if ! command -v jq >/dev/null 2>&1; then
    echo "FAIL: 356-show-multi-policy: jq not found; install jq to run JSON validation tests" >&2
    exit 1
fi
...
while [[ ! -S "$SOCKET_PATH" ]]; do
    if ! kill -0 "$DAEMON_PID" 2>/dev/null; then
        echo "FAIL: 356-show-multi-policy: daemon exited before socket appeared" >&2
        exit 1
    fi
    if (( SOCKET_WAIT >= 50 )); then
        echo "FAIL: 356-show-multi-policy: daemon socket did not appear within 5 seconds" >&2
        exit 1
    fi
    sleep 0.1
    (( SOCKET_WAIT++ )) || true
...
if [[ $APPLY_EXIT -ne 0 ]]; then
    echo "FAIL: 356-show-multi-policy: netfyr apply exited with code $APPLY_EXIT" >&2
    exit 1
fi
...
if ! echo "$SHOW_OUTPUT" | grep -q "e2e-mtu"; then
    echo "FAIL: 356-show-multi-policy: show output does not contain 'e2e-mtu'" >&2
    echo "      output: $SHOW_OUTPUT" >&2
    exit 1
fi
if ! echo "$SHOW_OUTPUT" | grep -q "e2e-dhcp"; then
    echo "FAIL: 356-show-multi-policy: show output does not contain 'e2e-dhcp'" >&2
    echo "      output: $SHOW_OUTPUT" >&2
    exit 1
fi
...
if ! echo "$SHOW_OUTPUT" | grep -q "(static)"; then
    echo "FAIL: 356-show-multi-policy: show output does not contain '(static)'" >&2
    echo "      output: $SHOW_OUTPUT" >&2
    exit 1
fi
if ! echo "$SHOW_OUTPUT" | grep -q "(dhcpv4)"; then
    echo "FAIL: 356-show-multi-policy: show output does not contain '(dhcpv4)'" >&2
    echo "      output: $SHOW_OUTPUT" >&2
    exit 1
fi
...
if ! echo "$SHOW_OUTPUT" | grep -q "DHCP:.*running"; then
    echo "FAIL: 356-show-multi-policy: show output does not contain 'DHCP:.*running'" >&2
    echo "      output: $SHOW_OUTPUT" >&2
    exit 1
fi
...
if ! echo "$SHOW_OUTPUT" | grep -q "120s total"; then
    echo "FAIL: 356-show-multi-policy: show output does not contain '120s total'" >&2
    echo "      output: $SHOW_OUTPUT" >&2
    exit 1
fi
...
if ! echo "$JSON_OUTPUT" | jq -e 'type == "object"' >/dev/null 2>&1; then
    echo "FAIL: 356-show-multi-policy: show -o json output is not a JSON object" >&2
...
if [[ -z "$MULTI_IFACE" ]]; then
    echo "FAIL: 356-show-multi-policy: veth-multi0 not found in interfaces array" >&2
...
if [[ "$POLICY_COUNT" -ne 2 ]]; then
    echo "FAIL: 356-show-multi-policy: veth-multi0 has $POLICY_COUNT policies, expected 2" >&2
...
if [[ "$HAS_STATIC" -ne 1 ]]; then
    echo "FAIL: 356-show-multi-policy: expected 1 static policy, got $HAS_STATIC" >&2
    exit 1
fi
if [[ "$HAS_DHCPV4" -ne 1 ]]; then
    echo "FAIL: 356-show-multi-policy: expected 1 dhcpv4 policy, got $HAS_DHCPV4" >&2
    exit 1
fi

echo "PASS: 356-show-multi-policy"
