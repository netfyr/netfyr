#!/bin/bash
# 351-journal-apply-entry.sh -- Standalone apply records a correct journal entry.
#
# Tests the standalone (daemon-free) journal write path in apply.rs.
# No daemon is started; NETFYR_SOCKET_PATH is pointed at a nonexistent path
# to force daemon-free mode.
#
# Requires: unshare, ip (iproute2), jq
# Usage:
#   NETFYR_BIN=./target/debug/netfyr \
#   bash tests/351-journal-apply-entry.sh
set -euo pipefail
...
if [[ ! -x "$NETFYR_BIN" ]]; then
    echo "FAIL: 351-journal-apply-entry: netfyr binary not found at $NETFYR_BIN" >&2
    exit 1
fi
if ! command -v jq >/dev/null 2>&1; then
    echo "FAIL: 351-journal-apply-entry: 'jq' not found; install jq to run this test" >&2
    exit 1
fi
...
# Point socket at a nonexistent path to guarantee daemon-free mode.
...
if [[ $APPLY_EXIT -ne 0 ]]; then
    echo "FAIL: 351-journal-apply-entry: netfyr apply exited with code $APPLY_EXIT" >&2
    exit 1
fi

if [[ ! -f "$JOURNAL_DIR/current.ndjson" ]]; then
    echo "FAIL: 351-journal-apply-entry: current.ndjson not found in journal directory" >&2
    exit 1
fi
...
if [[ "$APPLY_ENTRY" == "null" || -z "$APPLY_ENTRY" ]]; then
    echo "FAIL: 351-journal-apply-entry: no policy_apply entry found in journal" >&2
...
if [[ "$TRIGGER_TYPE" != "policy_apply" ]]; then
    echo "FAIL: 351-journal-apply-entry: expected trigger.type=policy_apply, got $TRIGGER_TYPE" >&2
    exit 1
fi
...
if [[ "$MTU_CHANGE_COUNT" -lt 1 ]]; then
    echo "FAIL: 351-journal-apply-entry: diff does not contain an mtu field change for veth-e2e0" >&2
...
if [[ "$MTU_AFTER" != "1400" ]]; then
    echo "FAIL: 351-journal-apply-entry: state_after.mtu expected 1400, got $MTU_AFTER" >&2
...
if [[ "$OUTCOME_KIND" != "applied" ]]; then
    echo "FAIL: 351-journal-apply-entry: expected outcome.kind=applied, got $OUTCOME_KIND" >&2
    exit 1
fi
if [[ "$OUTCOME_SUCCEEDED" -lt 1 ]]; then
    echo "FAIL: 351-journal-apply-entry: expected outcome.succeeded >= 1, got $OUTCOME_SUCCEEDED" >&2
    exit 1
fi

echo "PASS: 351-journal-apply-entry"
