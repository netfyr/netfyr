#!/bin/bash
# 600-e2e-show-json-waiting.sh -- End-to-end: netfyr show -o json omits lease fields for waiting factory.
#
...
if [[ ! -x "$NETFYR_BIN" ]]; then
    echo "FAIL: 600-e2e-show-json-waiting: netfyr binary not found at $NETFYR_BIN" >&2
    exit 1
fi
if [[ ! -x "$NETFYR_DAEMON_BIN" ]]; then
    echo "FAIL: 600-e2e-show-json-waiting: netfyr-daemon binary not found at $NETFYR_DAEMON_BIN" >&2
    exit 1
fi
if ! command -v jq >/dev/null 2>&1; then
    echo "FAIL: 600-e2e-show-json-waiting: jq not found; install jq to run JSON validation tests" >&2
    exit 1
fi
...
while [[ ! -S "$SOCKET_PATH" ]]; do
    if ! kill -0 "$DAEMON_PID" 2>/dev/null; then
        echo "FAIL: 600-e2e-show-json-waiting: daemon exited before socket appeared" >&2
        exit 1
    fi
    if (( SOCKET_WAIT >= 50 )); then
        echo "FAIL: 600-e2e-show-json-waiting: daemon socket did not appear within 5 seconds" >&2
        exit 1
    fi
    sleep 0.1
    (( SOCKET_WAIT++ )) || true
...
if [[ $APPLY_EXIT -ne 0 ]]; then
    echo "FAIL: 600-e2e-show-json-waiting: netfyr apply exited with code $APPLY_EXIT" >&2
    exit 1
fi
...
while true; do
    ...
    if (( WAIT_ITERS >= 300 )); then
        echo "FAIL: 600-e2e-show-json-waiting: DHCP factory did not enter 'waiting' state within 30 seconds" >&2
...
if ! echo "$JSON_OUTPUT" | jq -e 'type == "object"' >/dev/null 2>&1; then
    echo "FAIL: 600-e2e-show-json-waiting: JSON output is not an object" >&2
    exit 1
fi
...
if [[ -z "$DHCP_IFACE" ]]; then
    echo "FAIL: 600-e2e-show-json-waiting: veth-dhcp0 not found in interfaces array" >&2
    echo "      JSON: $JSON_OUTPUT" >&2
    exit 1
fi
...
if [[ "$DHCP_STATE" != "waiting" ]]; then
    echo "FAIL: 600-e2e-show-json-waiting: dhcp.state is '$DHCP_STATE', expected 'waiting'" >&2
    exit 1
fi
...
if [[ "$HAS_LEASE_TIME" != "false" ]]; then
    echo "FAIL: 600-e2e-show-json-waiting: dhcp object must not have 'lease_time_secs' for waiting factory" >&2
...
if [[ "$HAS_REMAINING" != "false" ]]; then
    echo "FAIL: 600-e2e-show-json-waiting: dhcp object must not have 'lease_remaining_secs' for waiting factory" >&2
    exit 1
fi
...
if [[ "$HAS_ADDR" != "false" ]]; then
    echo "FAIL: 600-e2e-show-json-waiting: dhcp object must not have 'lease_address' for waiting factory" >&2
    exit 1
fi

echo "PASS: 600-e2e-show-json-waiting"
