#!/bin/bash
# 356-show-unmanaged.sh -- End-to-end: netfyr show lists all interfaces including unmanaged ones.
#
...
if [[ ! -x "$NETFYR_BIN" ]]; then
    echo "FAIL: 356-show-unmanaged: netfyr binary not found at $NETFYR_BIN" >&2
    exit 1
fi
if [[ ! -x "$NETFYR_DAEMON_BIN" ]]; then
    echo "FAIL: 356-show-unmanaged: netfyr-daemon binary not found at $NETFYR_DAEMON_BIN" >&2
    exit 1
fi
if ! command -v dnsmasq >/dev/null 2>&1; then
    echo "FAIL: 356-show-unmanaged: dnsmasq not found; install dnsmasq to run DHCP tests" >&2
    exit 1
fi
...
while [[ ! -S "$SOCKET_PATH" ]]; do
    if ! kill -0 "$DAEMON_PID" 2>/dev/null; then
        echo "FAIL: 356-show-unmanaged: daemon exited before socket appeared" >&2
        exit 1
    fi
    if (( SOCKET_WAIT >= 50 )); then
        echo "FAIL: 356-show-unmanaged: daemon socket did not appear within 5 seconds" >&2
        exit 1
    fi
    sleep 0.1
    (( SOCKET_WAIT++ )) || true
...
if [[ $APPLY_EXIT -ne 0 ]]; then
    echo "FAIL: 356-show-unmanaged: netfyr apply exited with code $APPLY_EXIT" >&2
    exit 1
fi
...
if ! echo "$SHOW_OUTPUT" | grep -q "veth-managed0"; then
    echo "FAIL: 356-show-unmanaged: show output does not contain 'veth-managed0'" >&2
    echo "      output: $SHOW_OUTPUT" >&2
    exit 1
fi
if ! echo "$SHOW_OUTPUT" | grep -q "(static)"; then
    echo "FAIL: 356-show-unmanaged: show output does not contain '(static)' for veth-managed0" >&2
    echo "      output: $SHOW_OUTPUT" >&2
    exit 1
fi
...
if ! echo "$SHOW_OUTPUT" | grep -q "veth-dhcp0"; then
    echo "FAIL: 356-show-unmanaged: show output does not contain 'veth-dhcp0'" >&2
    echo "      output: $SHOW_OUTPUT" >&2
    exit 1
fi
if ! echo "$SHOW_OUTPUT" | grep -q "(dhcpv4)"; then
    echo "FAIL: 356-show-unmanaged: show output does not contain '(dhcpv4)'" >&2
    echo "      output: $SHOW_OUTPUT" >&2
    exit 1
fi
...
if ! echo "$SHOW_OUTPUT" | grep -q "veth-other0"; then
    echo "FAIL: 356-show-unmanaged: show output does not contain unmanaged 'veth-other0'" >&2
    echo "      output: $SHOW_OUTPUT" >&2
    exit 1
fi
if ! echo "$SHOW_OUTPUT" | grep -q "veth-managed1"; then
    echo "FAIL: 356-show-unmanaged: show output does not contain unmanaged peer 'veth-managed1'" >&2
    echo "      output: $SHOW_OUTPUT" >&2
    exit 1
fi

echo "PASS: 356-show-unmanaged"
