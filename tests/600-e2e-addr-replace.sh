#!/bin/bash
# 600-e2e-addr-replace.sh -- End-to-end: replacing the address set removes old addresses.
#
# Requires: unshare, ip (iproute2)
# Usage:
#   NETFYR_BIN=./target/debug/netfyr \
#   NETFYR_DAEMON_BIN=./target/debug/netfyr-daemon \
#   bash tests/600-e2e-addr-replace.sh
set -euo pipefail
...
if [[ ! -x "$NETFYR_BIN" ]]; then
    echo "FAIL: 600-e2e-addr-replace: netfyr binary not found at $NETFYR_BIN" >&2
    exit 1
fi
if [[ ! -x "$NETFYR_DAEMON_BIN" ]]; then
    echo "FAIL: 600-e2e-addr-replace: netfyr-daemon binary not found at $NETFYR_DAEMON_BIN" >&2
    exit 1
fi
...
while [[ ! -S "$SOCKET_PATH" ]]; do
    if ! kill -0 "$DAEMON_PID" 2>/dev/null; then
        echo "FAIL: 600-e2e-addr-replace: daemon exited before socket appeared" >&2
        exit 1
    fi
    if (( SOCKET_WAIT >= 50 )); then
        echo "FAIL: 600-e2e-addr-replace: daemon socket did not appear within 5 seconds" >&2
        exit 1
    fi
    sleep 0.1
    (( SOCKET_WAIT++ )) || true
...
if [[ $APPLY_A_EXIT -ne 0 ]]; then
    echo "FAIL: 600-e2e-addr-replace: first apply exited with code $APPLY_A_EXIT" >&2
    exit 1
fi
...
if [[ $APPLY_B_EXIT -ne 0 ]]; then
    echo "FAIL: 600-e2e-addr-replace: second apply exited with code $APPLY_B_EXIT" >&2
    exit 1
fi
...
assert_not_has_address veth-addr0 "10.99.0.4/24"
assert_not_has_address veth-addr0 "10.99.0.5/24"

# Verify new addresses are present.
...
# Verify ordering of the new addresses via netfyr query -o json.
...
echo "PASS: 600-e2e-addr-replace"
