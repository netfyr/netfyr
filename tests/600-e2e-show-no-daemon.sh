#!/bin/bash
# 600-e2e-show-no-daemon.sh -- End-to-end: netfyr show works gracefully when daemon is not running.
#
...
if [[ ! -x "$NETFYR_BIN" ]]; then
    echo "FAIL: 600-e2e-show-no-daemon: netfyr binary not found at $NETFYR_BIN" >&2
    exit 1
fi
...
if [[ $SHOW_EXIT -ne 0 ]]; then
    echo "FAIL: 600-e2e-show-no-daemon: netfyr show exited with code $SHOW_EXIT (expected 0)" >&2
...
if ! echo "$SHOW_OUTPUT" | grep -q "Status:  not running"; then
    echo "FAIL: 600-e2e-show-no-daemon: show output does not contain 'Status:  not running'" >&2
    echo "      output: $SHOW_OUTPUT" >&2
    exit 1
fi
...
if ! echo "$SHOW_OUTPUT" | grep -q "veth-e2e0"; then
    echo "FAIL: 600-e2e-show-no-daemon: show output does not list 'veth-e2e0'" >&2
    echo "      output: $SHOW_OUTPUT" >&2
    exit 1
fi
...
if echo "$SHOW_OUTPUT" | grep -q "Policies:"; then
    echo "FAIL: 600-e2e-show-no-daemon: show output unexpectedly contains 'Policies:' line" >&2
    echo "      output: $SHOW_OUTPUT" >&2
    exit 1
fi
if echo "$SHOW_OUTPUT" | grep -q "DHCP:"; then
    echo "FAIL: 600-e2e-show-no-daemon: show output unexpectedly contains 'DHCP:' line" >&2
    echo "      output: $SHOW_OUTPUT" >&2
    exit 1
fi
if echo "$SHOW_OUTPUT" | grep -q "Lease:"; then
    echo "FAIL: 600-e2e-show-no-daemon: show output unexpectedly contains 'Lease:' line" >&2
    echo "      output: $SHOW_OUTPUT" >&2
    exit 1
fi

echo "PASS: 600-e2e-show-no-daemon"
