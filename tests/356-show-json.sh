#!/bin/bash
# 356-show-json.sh -- End-to-end: netfyr show -o json produces valid JSON with DHCP lease data.
#
...
if [[ ! -x "$NETFYR_BIN" ]]; then
    echo "FAIL: 356-show-json: netfyr binary not found at $NETFYR_BIN" >&2
    exit 1
fi
if [[ ! -x "$NETFYR_DAEMON_BIN" ]]; then
    echo "FAIL: 356-show-json: netfyr-daemon binary not found at $NETFYR_DAEMON_BIN" >&2
    exit 1
fi
if ! command -v dnsmasq >/dev/null 2>&1; then
    echo "FAIL: 356-show-json: dnsmasq not found; install dnsmasq to run DHCP tests" >&2
    exit 1
fi
if ! command -v jq >/dev/null 2>&1; then
    echo "FAIL: 356-show-json: jq not found; install jq to run JSON validation tests" >&2
    exit 1
fi
...
while [[ ! -S "$SOCKET_PATH" ]]; do
    if ! kill -0 "$DAEMON_PID" 2>/dev/null; then
        echo "FAIL: 356-show-json: daemon exited before socket appeared" >&2
        exit 1
    fi
    if (( SOCKET_WAIT >= 50 )); then
        echo "FAIL: 356-show-json: daemon socket did not appear within 5 seconds" >&2
        exit 1
    fi
    sleep 0.1
    (( SOCKET_WAIT++ )) || true
...
if [[ $APPLY_EXIT -ne 0 ]]; then
    echo "FAIL: 356-show-json: netfyr apply exited with code $APPLY_EXIT" >&2
    exit 1
fi
...
if ! echo "$JSON_OUTPUT" | jq . >/dev/null 2>&1; then
    echo "FAIL: 356-show-json: show -o json output is not valid JSON" >&2
    echo "      output: $JSON_OUTPUT" >&2
    exit 1
fi
if ! echo "$JSON_OUTPUT" | jq -e 'type == "object"' >/dev/null 2>&1; then
    echo "FAIL: 356-show-json: JSON output is not an object" >&2
    exit 1
fi
...
if [[ "$DAEMON_STATUS" != "running" ]]; then
    echo "FAIL: 356-show-json: daemon.status is '$DAEMON_STATUS', expected 'running'" >&2
    exit 1
fi
...
if [[ "$UPTIME" == "null" ]] || [[ "$UPTIME" -lt 0 ]]; then
    echo "FAIL: 356-show-json: daemon.uptime_seconds is '$UPTIME', expected non-negative integer" >&2
    exit 1
fi
...
if [[ "$IFACE_COUNT" -lt 1 ]]; then
    echo "FAIL: 356-show-json: interfaces array is empty" >&2
    exit 1
fi
...
if [[ -z "$DHCP_IFACE" ]]; then
    echo "FAIL: 356-show-json: veth-dhcp0 not found in interfaces array" >&2
...
if [[ "$POLICY_NAME" != "e2e-show-json" ]]; then
    echo "FAIL: 356-show-json: policy 'e2e-show-json' not found in veth-dhcp0 policies" >&2
    exit 1
fi
if [[ "$POLICY_TYPE" != "dhcpv4" ]]; then
    echo "FAIL: 356-show-json: policy type is '$POLICY_TYPE', expected 'dhcpv4'" >&2
    exit 1
fi
...
if [[ "$DHCP_STATE" != "running" ]]; then
    echo "FAIL: 356-show-json: dhcp.state is '$DHCP_STATE', expected 'running'" >&2
    exit 1
fi
...
if ! echo "$LEASE_ADDR" | grep -q "10.99.1."; then
    echo "FAIL: 356-show-json: dhcp.lease_address '$LEASE_ADDR' does not contain '10.99.1.'" >&2
    exit 1
fi
if ! echo "$LEASE_ADDR" | grep -q "/"; then
    echo "FAIL: 356-show-json: dhcp.lease_address '$LEASE_ADDR' does not contain '/'" >&2
    exit 1
fi
...
if [[ "$LEASE_TIME" != "120" ]]; then
    echo "FAIL: 356-show-json: dhcp.lease_time_secs is '$LEASE_TIME', expected 120" >&2
    exit 1
fi
...
if [[ "$LEASE_REMAINING" == "null" ]] || [[ "$LEASE_REMAINING" -lt 0 ]] || [[ "$LEASE_REMAINING" -gt 120 ]]; then
    echo "FAIL: 356-show-json: dhcp.lease_remaining_secs is '$LEASE_REMAINING', expected integer in [0, 120]" >&2
    exit 1
fi

echo "PASS: 356-show-json"
