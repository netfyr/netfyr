#!/bin/bash
# 600-e2e-addr-five.sh -- End-to-end: five addresses applied and verified in order.
#
# Requires: unshare, ip (iproute2)
# Usage:
#   NETFYR_BIN=./target/debug/netfyr \
#   NETFYR_DAEMON_BIN=./target/debug/netfyr-daemon \
#   bash tests/600-e2e-addr-five.sh
set -euo pipefail
...
if [[ ! -x "$NETFYR_BIN" ]]; then
    echo "FAIL: 600-e2e-addr-five: netfyr binary not found at $NETFYR_BIN" >&2
    exit 1
fi
if [[ ! -x "$NETFYR_DAEMON_BIN" ]]; then
    echo "FAIL: 600-e2e-addr-five: netfyr-daemon binary not found at $NETFYR_DAEMON_BIN" >&2
    exit 1
fi
...
while [[ ! -S "$SOCKET_PATH" ]]; do
    if ! kill -0 "$DAEMON_PID" 2>/dev/null; then
        echo "FAIL: 600-e2e-addr-five: daemon exited before socket appeared" >&2
        exit 1
    fi
    if (( SOCKET_WAIT >= 50 )); then
        echo "FAIL: 600-e2e-addr-five: daemon socket did not appear within 5 seconds" >&2
        exit 1
    fi
    sleep 0.1
    (( SOCKET_WAIT++ )) || true
...
if [[ $APPLY_EXIT -ne 0 ]]; then
    echo "FAIL: 600-e2e-addr-five: netfyr apply exited with code $APPLY_EXIT" >&2
    exit 1
fi
