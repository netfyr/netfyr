#!/bin/bash
# 600-e2e-show-mixed.sh -- End-to-end: netfyr show displays both static and DHCP managed interfaces.
#
...
if [[ ! -x "$NETFYR_BIN" ]]; then
    echo "FAIL: 600-e2e-show-mixed: netfyr binary not found at $NETFYR_BIN" >&2
    exit 1
fi
if [[ ! -x "$NETFYR_DAEMON_BIN" ]]; then
    echo "FAIL: 600-e2e-show-mixed: netfyr-daemon binary not found at $NETFYR_DAEMON_BIN" >&2
    exit 1
fi
if ! command -v dnsmasq >/dev/null 2>&1; then
    echo "FAIL: 600-e2e-show-mixed: dnsmasq not found; install dnsmasq to run DHCP tests" >&2
    exit 1
fi
if ! command -v jq >/dev/null 2>&1; then
    echo "FAIL: 600-e2e-show-mixed: jq not found; install jq to run JSON validation tests" >&2
    exit 1
fi
...
while [[ ! -S "$SOCKET_PATH" ]]; do
    if ! kill -0 "$DAEMON_PID" 2>/dev/null; then
        echo "FAIL: 600-e2e-show-mixed: daemon exited before socket appeared" >&2
        exit 1
    fi
    if (( SOCKET_WAIT >= 50 )); then
        echo "FAIL: 600-e2e-show-mixed: daemon socket did not appear within 5 seconds" >&2
        exit 1
    fi
    sleep 0.1
    (( SOCKET_WAIT++ )) || true
...
if [[ $APPLY_EXIT -ne 0 ]]; then
    echo "FAIL: 600-e2e-show-mixed: netfyr apply exited with code $APPLY_EXIT" >&2
    exit 1
fi
...
if ! echo "$SHOW_OUTPUT" | grep -q "veth-static0"; then
    echo "FAIL: 600-e2e-show-mixed: show output does not contain 'veth-static0'" >&2
    echo "      output: $SHOW_OUTPUT" >&2
    exit 1
fi
if ! echo "$SHOW_OUTPUT" | grep -q "(static)"; then
    echo "FAIL: 600-e2e-show-mixed: show output does not contain '(static)' policy type" >&2
    echo "      output: $SHOW_OUTPUT" >&2
    exit 1
fi
...
if ! echo "$SHOW_OUTPUT" | grep -q "veth-dhcp0"; then
    echo "FAIL: 600-e2e-show-mixed: show output does not contain 'veth-dhcp0'" >&2
    echo "      output: $SHOW_OUTPUT" >&2
    exit 1
fi
if ! echo "$SHOW_OUTPUT" | grep -q "(dhcpv4)"; then
    echo "FAIL: 600-e2e-show-mixed: show output does not contain '(dhcpv4)' policy type" >&2
    echo "      output: $SHOW_OUTPUT" >&2
    exit 1
fi
if ! echo "$SHOW_OUTPUT" | grep -q "DHCP:.*running"; then
    echo "FAIL: 600-e2e-show-mixed: show output does not contain 'DHCP:.*running'" >&2
    echo "      output: $SHOW_OUTPUT" >&2
    exit 1
fi
if ! echo "$SHOW_OUTPUT" | grep -q "120s total"; then
    echo "FAIL: 600-e2e-show-mixed: show output does not contain '120s total'" >&2
    echo "      output: $SHOW_OUTPUT" >&2
    exit 1
fi
...
if ! echo "$SHOW_OUTPUT" | grep -q "veth-static1"; then
    echo "FAIL: 600-e2e-show-mixed: show output does not contain unmanaged 'veth-static1'" >&2
    echo "      output: $SHOW_OUTPUT" >&2
    exit 1
fi
...
if ! echo "$JSON_OUTPUT" | jq -e 'type == "object"' >/dev/null 2>&1; then
    echo "FAIL: 600-e2e-show-mixed: show -o json output is not a JSON object" >&2
...
if [[ "$DAEMON_STATUS" != "running" ]]; then
    echo "FAIL: 600-e2e-show-mixed: daemon.status is '$DAEMON_STATUS', expected 'running'" >&2
    exit 1
fi

IFACE_COUNT=$(echo "$JSON_OUTPUT" | jq '.interfaces | length')
if [[ "$IFACE_COUNT" -lt 2 ]]; then
    echo "FAIL: 600-e2e-show-mixed: interfaces array has $IFACE_COUNT elements, expected at least 2" >&2
    exit 1
fi
...
if [[ "$DHCP_STATE" != "running" ]]; then
    echo "FAIL: 600-e2e-show-mixed: veth-dhcp0 dhcp.state is '$DHCP_STATE', expected 'running'" >&2
    exit 1
fi

echo "PASS: 600-e2e-show-mixed"
